...
import 'package:flutter_ui/Quran%20App/models/asma_un_nabi_model.dart';

import '../utils/colors.dart';

class AsmaUnNabiPage extends StatefulWidget {
  const AsmaUnNabiPage({super.key});

  @override
  State<AsmaUnNabiPage> createState() => _AsmaUnNabiPageState();
}

class _AsmaUnNabiPageState extends State<AsmaUnNabiPage> {
  List<AsmaaUnNabiModel> nameLists = [];
  ...
  Future<void> asmaaUnNabiNames() async {
    final String response = await rootBundle.loadString(
      'assets/data/quran_data.json',
    );
    final Map<String, dynamic> data = json.decode(response);
    final List<dynamic> list = data['asmaaUnNabi'];

    setState(() {
      nameLists = list.map((e) => AsmaaUnNabiModel.fromJson(e)).toList();
    });
  }
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            const Text(
              "99 Beautiful Names of Prophet (S.A.W)",
  ...
  Widget _NameGridCard({required AsmaaUnNabiModel name}) {
...
void _showNameModal(BuildContext context, AsmaaUnNabiModel name) {
